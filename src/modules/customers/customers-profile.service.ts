...
async function getPartyType(tenantId: string, customerId: string): Promise<string> {
  const db = getDbClient();
  const row = await db.get<{ party_type: string }>(
    'SELECT party_type FROM customers WHERE id = ? AND tenant_id = ?',
    [customerId, tenantId]
  );
...
export const customersProfileService = {
  async upsert(tenantId: string, customerId: string, input: UpsertProfileInput): Promise<CustomerProfile> {
    const db = getDbClient();
    const partyType = await getPartyType(tenantId, customerId);
    ...
    if (partyType === 'natural_person') {
      ...
      await db.run(
        ...
        [`${p.given_name} ${p.family_name}`, now, customerId]
      );
    } else {
      const p = input as { partyType: 'legal_entity' } & UpsertLegalEntityProfileInput;
      await db.run(
        'UPDATE customers SET display_name = ?, updated_at = ? WHERE id = ?',
        [p.legal_name, now, customerId]
      );
    }

    return (await customersProfileService.get(tenantId, customerId))!;
  },

  async get(tenantId: string, customerId: string): Promise<CustomerProfile | null> {
    const db = getDbClient();
    const partyType = await getPartyType(tenantId, customerId);
