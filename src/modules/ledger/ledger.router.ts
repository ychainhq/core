...
ledgerRouter.post('/accounts', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!await db.get('SELECT id FROM chains WHERE id = ?', [body.chainId])) {
      throw new NotFoundError('Chain', body.chainId);
    }
    if (!await db.get('SELECT id FROM assets WHERE id = ?', [body.assetId])) {
      throw new NotFoundError('Asset', body.assetId);
    }

    const account = await ledgerService.createAccount(tenantId, body);
...
ledgerRouter.get('/accounts', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await ledgerService.listAccounts(tenantId, query);
    res.json({
      data: result.data,
      pagination: {
...
ledgerRouter.get('/accounts/:ledgerAccountId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = (req as any).tenantId as string;
    const account = await ledgerService.getAccountById(tenantId, req.params['ledgerAccountId']!);
...
ledgerRouter.get('/accounts/:ledgerAccountId/balances', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = (req as any).tenantId as string;
    const accountId = req.params['ledgerAccountId']!;
    await ledgerService.getAccountById(tenantId, accountId); // validates exists and belongs to tenant
...
ledgerRouter.get('/accounts/:ledgerAccountId/entries', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = (req as any).tenantId as string;
    const accountId = req.params['ledgerAccountId']!;
    await ledgerService.getAccountById(tenantId, accountId); // validates exists and belongs to tenant
    const query = listEntriesQuerySchema.parse(req.query);
    const result = await ledgerService.listEntries(accountId, query);
    res.json({
      data: result.data,
      pagination: {
        limit: query.limit ?? 20,
...
ledgerRouter.post('/transfers', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (idempotencyKey) {
      const existing = await idempotencyService.get(tenantId, idempotencyKey, 'ledger_transfer');
      if (existing) {
    ...
    await ledgerService.getAccountById(tenantId, body.fromLedgerAccountId);
    await ledgerService.getAccountById(tenantId, body.toLedgerAccountId);

    const { debit, credit } = await ledgerService.transfer({
    ...
    if (idempotencyKey) {
      await idempotencyService.save(tenantId, idempotencyKey, 'ledger_transfer', result, 201);
    }
